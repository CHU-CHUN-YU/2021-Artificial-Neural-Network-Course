...
#read input & output data
x_train=open("D:/hw2/hw2_data/pm25/PM25/train_X.txt")
x_train=pd.read_csv(x_train, sep="\t",header=0)
y_train=open("D:/hw2/hw2_data/pm25/PM25/train_y.txt")
y_train=(pd.read_csv(y_train, sep="\t",header=0)).iloc[:,1]

x_test=open("D:/hw2/hw2_data/pm25/PM25/test_X.txt")
x_test=pd.read_csv(x_test, sep="\t",header=0)
y_test=open("D:/hw2/hw2_data/pm25/test_Y_PM25_real.txt")
...
checkpoint =ModelCheckpoint("D:/hw2/hw2_data/pm25/PM25/rbf-model.hdf5",save_best_only=True) 
...
model.summary()
model=load_model("D:hw2/hw2_data/pm25/PM25/rbf-model.hdf5", custom_objects={'RBFLayer': RBFLayer}) #把儲存好的最佳模式讀入
